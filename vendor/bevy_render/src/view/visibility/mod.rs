...
mod visibility_retry_tests {
    use super::*;
    use crate::camera::DirtySpecializations;
    use crate::view::RetainedViewEntity;

    fn out_of_order_entities() -> Vec<(Entity, MainEntity)> {
        let mut main_world = bevy_ecs::world::World::new();
        let mut render_world = bevy_ecs::world::World::new();
        let main: Vec<MainEntity> = (0..4)
            .map(|_| main_world.spawn_empty().id().into())
            .collect();
        let render: Vec<Entity> = (0..4).map(|_| render_world.spawn_empty().id()).collect();
        let mut pairs = vec![
            (render[3], main[0]),
            (render[0], main[1]),
            (render[2], main[2]),
        ...
        pairs.sort_unstable_by_key(|(_, entity)| *entity);
        pairs
    }

    #[test]
    fn cpu_visibility_lookup_uses_main_entity_sort_order() {
        let pairs = out_of_order_entities();
        let mut visible = RenderVisibleEntitiesClass::default();
        visible.update_cpu_culled_entities(&pairs);
        for &(render, main) in &pairs {
            assert!(
                visible.entity_pair_is_visible(render, main),
                "visible pair {render:?}/{main:?} was lost"
            );
            assert!(
                !visible.entity_pair_is_visible(Entity::PLACEHOLDER, main),
                "a stale render entity must not match"
            );
        }
    }

    #[test]
    fn pending_materials_survive_until_ready_without_visibility_changes() {
        let pairs = out_of_order_entities();
        let mut visible = RenderVisibleEntitiesClass::default();
        visible.update_cpu_culled_entities(&pairs);
        visible.prepare_for_new_frame();
        let pending = pairs.iter().copied().collect();
        let dirty = DirtySpecializations::default();
        let view = RetainedViewEntity {
            main_entity: pairs[0].1,
            auxiliary_entity: Entity::PLACEHOLDER.into(),
            subview_index: 0,
        };
        assert_eq!(
            dirty.iter_to_specialize(view, &visible, &pending).count(),
            pairs.len()
        );
        assert_eq!(
            dirty.iter_to_queue(view, &visible, &pending).count(),
            pairs.len()
        );
        visible.update_cpu_culled_entities(&pairs[1..]);
        assert_eq!(
            dirty.iter_to_queue(view, &visible, &pending).count(),
            pairs.len() - 1
        );
...
impl RenderVisibleEntitiesClass {
    ...
    pub fn entity_pair_is_visible(&self, entity: Entity, main_entity: MainEntity) -> bool {
        // CPU visibility is sorted by main entity; render IDs can arrive in any order.
        let visible = self
