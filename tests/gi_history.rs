//! Production GI history expires old radiance even after repeated spatial reuse.
...
fn gi_history_gpu() {
    futures_lite::future::block_on(async {
        ...
        let mut source = include_str!("gi_history_fixture.wgsl").to_owned();
        for name in [
            "empty_reservoir",
            "merge_reservoirs",
            "jacobian",
        ...
        for line in gi.lines().filter(|line| {
            line.starts_with("const MAX_GI_SAMPLE_AGE")
                || line.starts_with("const CONFIDENCE_WEIGHT_CAP")
        }) {
        ...
        for tint in [
            ...
            if tint[3] > 0.0 {
                assert_eq!(samples[2][1], 7.0, "spatial reuse rejuvenated old radiance");
            }
            assert_eq!(samples[2][2], 0.0, "expired history still has confidence");
